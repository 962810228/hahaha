...
//封装过滤敏感字体
function filter(Arr,mess,val)//遍历的数组,需要替换的字符串,替换的值
{
    //var Arr=["傻逼","脑残"];
    //var mess=msg.value;
	//var val="大可爱"；
...
//伪类数组Array
Array.prototype.toArray=function (arr)
{
	arr=Array.from(new Set(arr));
}
//封装日期
function GetTime(ele)//ele是渲染的元素
{
    var a=new Date();
    var year=a.getFullYear();
    var month=jia(a.getMonth()+1);
    var date=jia(a.getDate());
    var day=a.getDay();
    var hour=jia(a.getHours());
    var minute=jia(a.getMinutes());
    var seconds=jia(a.getSeconds());
    var str="日一二三四五六";
    // var ele=document.getElementById("shuru");
    ele.innerHTML="当前日期是："+year+"年"+month+"月"+date+"日"+"星期"+str[day]+hour+"时"+minute+"分"+seconds+"秒";
    function jia(num)
    {
        if(num<10)
        {
